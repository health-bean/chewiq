...
class ComprehensiveDocumentationGenerator {
  ...
  async generateComprehensiveDocumentation() {
    ...
    await Promise.all([
      this.analyzeCodebase(discoveredPaths),
      this.testAllAPIs(),
  ...
  async analyzeCodebase(discoveredPaths) {
  ...
  async findAllComponents(discoveredPaths) {
    ...
    if (discoveredPaths && discoveredPaths.componentDirs) {
      ...
      componentDirs = discoveredPaths.componentDirs
        .filter(dir => !dir.path.includes('archive') && !dir.path.includes('docs/build') && dir.componentCount > 0)
        .map(dir => path.join(this.rootPath, dir.path));
    } else {
  ...
  async findCustomHooks(discoveredPaths) {
    ...
    if (discoveredPaths && discoveredPaths.hookDirs) {
      hookDirs = discoveredPaths.hookDirs
        .filter(dir => dir.hookCount > 0)
        .map(dir => path.join(this.rootPath, dir.path));
    } else {
      // Fallback to scanning
      hookDirs = [
  ...
  async findUtilities(discoveredPaths) {
    ...
    if (discoveredPaths && discoveredPaths.utilDirs) {
      utilDirs = discoveredPaths.utilDirs
        .filter(dir => dir.utilCount > 0)
        .map(dir => path.join(this.rootPath, dir.path));
    } else {
      // Fallback to scanning
      utilDirs = [
  ...
  async analyzeEnvironmentVariables(discoveredPaths) {
    console.log('🔐 Analyzing environment variables...');
    
    let envFiles = [];
    
    if (discoveredPaths && discoveredPaths.envFiles) {
      envFiles = discoveredPaths.envFiles.map(env => env.path);
    } else {
      // Fallback to scanning
      envFiles = [
        '.env',
    ...
    if (discoveredPaths && discoveredPaths.packageJsonFiles) {
      packagePaths = discoveredPaths.packageJsonFiles.map(pkg => pkg.path);
    } else {
      // Fallback to scanning
      packagePaths = [
        'package.json',
  ...
  async analyzePackageFiles(discoveredPaths) {
