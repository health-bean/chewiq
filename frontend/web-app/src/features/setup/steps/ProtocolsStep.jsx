...
const ProtocolsStep = ({ setupData, updateSetupData, protocols, onNext, onBack, isLast }) => (
  <div className="space-y-6">
      ...
      {protocols.map((protocol) => (
        <label
          ...
          className={`flex items-start space-x-3 p-3 border rounded-lg cursor-pointer transition-colors ${
            setupData.protocols.includes(protocol.id) ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'
          }`}
        >
          <input
            type="checkbox"
    ...
    <div className="flex space-x-3">
      ...
      <Button 
        onClick={onNext} 
        disabled={setupData.protocols.length === 0}
        className="flex-1"
      >
